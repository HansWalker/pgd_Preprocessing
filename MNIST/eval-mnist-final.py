...
def get_data(train_batch_size):
    ...
    train_loader = DataLoader(datasets.MNIST(root=data_root, train=False, transform=data_transform, target_transform=None, download=True),
...
img_size = [28,28,1]
trained_model_path = 'trained_models/sargan_mnist-x1-1'
trained_model_path2 = 'trained_models/sargan_mnist-x2-1'
trained_model_path3 = 'trained_models/sargan_mnist-x3-1'
trained_model_path4 = 'trained_models/sargan_mnist-x4-1'
BATCH_SIZE = 64
    ...
    with gx1.as_default():
        with tf.Session() as sess2:
            ...
            for ibatch in range(num_batches):
                ...
                x_blur_list1.append(blurred_batch.reshape([len(blurred_batch),img_size[0]*img_size[1]]))
                x_adv_list1.append(processed_batch.reshape([len(processed_batch),img_size[0]*img_size[1]]))
    with gx2.as_default():
        with tf.Session() as sessx2:
            ...
            for ibatch in range(num_batches):
                ...
                x_blur_list2.append(blurred_batch.reshape([len(blurred_batch),img_size[0]*img_size[1]]))
                x_adv_list2.append(processed_batch.reshape([len(processed_batch),img_size[0]*img_size[1]]))
    with gx3.as_default():
        with tf.Session() as sessx3:
            ...
            for ibatch in range(num_batches):
                ...
                x_blur_list3.append(blurred_batch.reshape([len(blurred_batch),img_size[0]*img_size[1]]))
                x_adv_list3.append(processed_batch.reshape([len(processed_batch),img_size[0]*img_size[1]]))
    ...
    with gx4.as_default():
        with tf.Session() as sessx4:
            ...
            for ibatch in range(num_batches):
                ...
                x_adv_list4[ibatch]=processed_batch.reshape([len(processed_batch),img_size[0]*img_size[1]])
                ...
                x_blur_list4[ibatch]=blurred_batch.reshape([len(blurred_batch),img_size[0]*img_size[1]])
            psnr=0
            for jj in range(num_batches):
                next_psnr=0
                psnr_value=(tf.image.psnr(np.array(x_batch_list[jj]).reshape([64,img_size[0],img_size[1],img_size[2]]),np.array(x_adv_list4[jj]).reshape([64,img_size[0],img_size[1],img_size[2]]),max_val=1))
