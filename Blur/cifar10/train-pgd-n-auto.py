...
model_dir = "pgd/models/cifar10-n-auto"
...
with tf.Session() as sess:
    ...
    training_time = 0.0
    # Main training loop
    for ii in range(int(max_num_training_steps/(num_checkpoint_steps*20))):
        allx_data, labels, allo_data=transform_data(num_checkpoint_steps*20,batch_size)
        for jj in range(num_checkpoint_steps*20):            
            ...
            adv_dict = {model.x_input: x_batch_adv,
                ...
                # Output to stdout
            if (ii*num_checkpoint_steps*20+jj) % num_output_steps == 0:
                nat_acc = sess.run(model.accuracy, feed_dict=nat_dict)
                adv_acc = sess.run(model.accuracy, feed_dict=adv_dict)
                print('Step {}:    ({})'.format((ii*20*num_checkpoint_steps+jj), datetime.now()))
                print('    training nat accuracy {:.4}%'.format(nat_acc * 100))
                print('    training adv accuracy {:.4}%'.format(adv_acc * 100))
                if (ii*20*num_checkpoint_steps+jj) != 0:
            ...
            # Write a checkpoint
            if (ii*20*num_checkpoint_steps+jj) % num_checkpoint_steps == 0:
                saver.save(sess,
