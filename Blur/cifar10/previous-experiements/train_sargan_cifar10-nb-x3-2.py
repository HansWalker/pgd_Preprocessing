...
output_dir = 'outputs'
trained_model_path = 'trained_models/sargan_cifar10-nb-x3-2'
trained_model_path2 = 'trained_models/sargan_cifar10-nb-2'
trained_model_path3 = 'trained_models/sargan_cifar10-nb-x2-2'
        ...
        with tf.Session() as sess2:
            ...
            for i in range(NUM_ITERATION):
                ...
                for k in range(len(features)):
                    features[k]=add_gaussian_noise(features[k], sd=np.random.uniform(NOISE_STD_RANGE[0], NOISE_STD_RANGE[0]))
                processed_batch=sess2.run(sargan_model.gen_img,feed_dict={sargan_model.image: features, sargan_model.cond: features})
                for k in range(len(processed_batch)):
                    processed_batch[k]=add_gaussian_noise(processed_batch[k], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))
    ...
    with g3.as_default():
        with tf.Session() as sess3:
            ...
            for i in range(NUM_ITERATION):
                ...
                for k in range(len(encoded_data[i])):
                    encoded_data[i][k]=add_gaussian_noise(encoded_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))
            for i in range(NUM_TEST_PER_EPOCH):
                val_data[i]=sess3.run(sargan_model2.gen_img,feed_dict={sargan_model2.image: val_data[i], sargan_model2.cond: val_data[i]})
                for k in range(len(encoded_data[i])):
                    val_data[i][k]=add_gaussian_noise(val_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))
