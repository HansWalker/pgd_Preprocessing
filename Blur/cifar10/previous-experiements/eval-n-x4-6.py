...
img_size = [32,32,1]
trained_model_path = 'trained_models/sargan_cifar10-x1-6'
trained_model_path2 = 'trained_models/sargan_cifar10-x2-6'
trained_model_path3 = 'trained_models/sargan_cifar10-x3-6'
trained_model_path4 = 'trained_models/sargan_cifar10-x4-6'
BATCH_SIZE = 64
...
def evaluate_checkpoint(filename):
    ...
    with gx1.as_default():
        with tf.Session() as sess2:
            ...
            for ibatch in range(num_batches):
                for k in range(len(x_adv_list[ibatch])):
                     x_adv_list[ibatch][k]=add_gaussian_noise(x_adv_list[ibatch][k], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))
                     x_blur_list[ibatch][k]=add_gaussian_noise(x_blur_list[ibatch][k], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))
