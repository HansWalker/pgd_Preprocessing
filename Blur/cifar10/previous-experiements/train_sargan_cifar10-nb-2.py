...
output_dir = 'outputs'
trained_model_path = 'trained_models/sargan_cifar10-nb-2'
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in progress_bar:
            ...
            for i in range (NUM_ITERATION):
                ...
                #corrupt the images
                corrupted_batch = np.array([add_gaussian_noise(image, sd=np.random.uniform(NOISE_STD_RANGE[0], NOISE_STD_RANGE[0])) for image in features_copy])
            ...
            for j in range(NUM_TEST_PER_EPOCH):
                ...
                #corrupt the images
                corrupted_batch = np.array([add_gaussian_noise(image, sd=np.random.uniform(NOISE_STD_RANGE[0], NOISE_STD_RANGE[0])) for image in batch_copy])
                #for i in range(len(corrupted_batch)):
                 #   corrupted_batch[i]=gaussian_filter(corrupted_batch[i], sigma=1)
