...
output_dir = 'outputs'
trained_model_path = 'trained_models/sargan_cifar10-x4-6'
trained_model_path2 = 'trained_models/sargan_cifar10-x1-6'
trained_model_path3 = 'trained_models/sargan_cifar10-x2-6'
trained_model_path4 = 'trained_models/sargan_cifar10-x3-6'
...
def transfrom_data(NUM_TEST_PER_EPOCH):
        ...
        with tf.Session() as sess2:
            ...
            for i in range(NUM_ITERATION):
                ...
                for j in range(len(features2)):
                    nextimage=Image.fromarray((features2[j]).astype(np.uint8))
                    nextimage=nextimage.convert('L')
                    features[j,:,:,0]=np.array(nextimage,dtype='float32')/255
                    original_images[j,:,:,0]=features[j,:,:,0]
    ...
    with g3.as_default():
        with tf.Session() as sess3:
            ...
            for i in range(NUM_ITERATION):
                for k in range(len(features)):
                    encoded_data[i][k]=add_gaussian_noise(encoded_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE2[0], NOISE_STD_RANGE2[1]))
            ...
            for i in range(NUM_TEST_PER_EPOCH):
                for k in range(len(features)):
                    val_data[i][k]=add_gaussian_noise(val_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE2[0], NOISE_STD_RANGE2[1]))
    ...
    with g4.as_default():
        with tf.Session() as sess4:
            ...
            for i in range(NUM_ITERATION):
                for k in range(len(features)):
                    encoded_data[i][k]=add_gaussian_noise(encoded_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE2[0], NOISE_STD_RANGE2[1]))
                encoded_data[i]=sess4.run(sargan_model4.gen_img,feed_dict={sargan_model4.image: encoded_data[i], sargan_model4.cond: encoded_data[i]})
            for i in range(NUM_TEST_PER_EPOCH):
                for k in range(len(features)):
                    val_data[i][k]=add_gaussian_noise(val_data[i][k], sd=np.random.uniform(NOISE_STD_RANGE2[0], NOISE_STD_RANGE2[1]))
                val_data[i]=sess4.run(sargan_model4.gen_img,feed_dict={sargan_model4.image: val_data[i], sargan_model4.cond: val_data[i]})
