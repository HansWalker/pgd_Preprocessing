...
def blur(batch):
    ...
    for i in range(len(newbatch)):
        corruptedbatch[i]=gaussian_filter(np.array([add_gaussian_noise(newbatch[i], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))]), sigma=1)[0]
...
trained_model_path2 = 'trained_models/sargan_cifar10x2-1'
BATCH_SIZE = 64
