...
def blur(batch):
    ...
    for i in range(len(newbatch)):
        corruptedbatch[i]=gaussian_filter(np.array([add_gaussian_noise(newbatch[i], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))]), sigma=.5)[0]
...
img_size = [32,32,1]
trained_model_path = 'trained_models/sargan_cifar10-32'
trained_model_path2 = 'trained_models/sargan_cifar10x2-32'
BATCH_SIZE = 64
...
def evaluate_checkpoint(filename):
    ...
    # Restore the checkpoint
        ...
        for ibatch in range(num_batches):
            ...
            x_blur_list.append(blur(x_batch))#.reshape([len(x_batch),img_size[0],img_size[1],img_size[2]]))
            x_adv_list.append(blur(x_batch_adv))#.reshape([len(x_batch),img_size[0],img_size[1],img_size[2]]))
