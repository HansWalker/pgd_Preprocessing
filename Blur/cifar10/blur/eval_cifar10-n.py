...
def blur(batch):
    ...
    for i in range(len(newbatch)):
        corruptedbatch[i]=gaussian_filter(np.array([add_gaussian_noise(newbatch[i], sd=np.random.uniform(NOISE_STD_RANGE[1], NOISE_STD_RANGE[1]))]), sigma=2)[0]
...
def evaluate_checkpoint(filename):
    ...
    # Restore the checkpoint
        ...
        for ibatch in range(num_batches):
            ...
            x_blur_list.append(blur(x_batch))#(x_batch.reshape([len(x_batch),img_size[0],img_size[1],img_size[2]]))
            x_adv_list.append(blur(x_batch_adv))#x_batch_adv.reshape([len(x_batch),img_size[0],img_size[1],img_size[2]]))
    ...
    with g2.as_default():
        with tf.Session() as sess2:
            ...
            for ibatch in range(num_batches):
                ...
                x_adv_list[ibatch]=processed_batch.reshape([len(x_adv_list[ibatch]),img_size[0]*img_size[1]])
                ...
                x_blur_list[ibatch]=blurred_batch.reshape([len(x_blur_list[ibatch]),img_size[0]*img_size[1]])
