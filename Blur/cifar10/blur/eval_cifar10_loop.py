    ...
    if eval_on_cpu:
      with tf.device("/cpu:0"):
        ...
        attack = LinfPGDAttack(model, 
                             ...
                             config['loss_func'])
        attacks.append(attack)
    epsilon+=.05/int(math.ceil(num_eval_examples / eval_batch_size))
...
def evaluate_checkpoint(filename):
    ...
    #           with tf.device("/cpu:0"):
            ...
            y_batch_list=[]
            x_adv_list=[]
            train_loader= get_data(eval_batch_size)
        ...
        np.save('loop_data/cifar10/cifar10_loop_list'+str(i)+".npy",fmnist_loop_list)
        np.save('loop_data/cifar10/cifar10_loop_list_corr'+str(i)+".npy",fmnist_loop_list_corr)
# Infinite eval loop
while True:
