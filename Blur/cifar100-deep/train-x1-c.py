...
output_dir = 'outputs'
trained_model_path = 'trained_models/sargan_cifar100-x1-c'
    ...
    with g2.as_default():
        with tf.Session() as sess2:
            ...
            for i in range(NUM_ITERATION):
                ...
                features=np.zeros([len(features2),img_size[0],img_size[1]*img_size[2],1])
                original_images=np.zeros([len(features2),img_size[0],img_size[1]*img_size[2],1])
                channel=0
                for j in range(len(features2)):
                    for k in range(img_size[1]):
                        for kk in range(img_size[2]):
                            if(kk%img_size[2]==0):
                                channel+=1
                            features[j,k,kk]=features2[j,((int)(k%img_size[2])),channel]
            ...
            for i in range(NUM_TEST_PER_EPOCH):
                ...
                features = np.array(features.data.numpy().transpose(0,2,3,1))
                original_images=np.zeros([len(features),img_size[0],img_size[1],1])
