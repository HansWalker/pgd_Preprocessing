...
trained_model_path1 = 'trained_models/sargan_cifar100-x1-1' 
...
def transform_data(num_batches,BATCH_SIZE):
        ...
        with tf.Session() as sess1:
            ...
            for i in range(num_batches):
                ...
                if(len(features)==BATCH_SIZE):
                    ...
                    original_data.append(features2.reshape([len(features),img_size[0]*img_size[1]]))
    ...
    return encoded_data, labels_list, original_data
...
with tf.Session() as sess:
    ...
    training_time = 0.0
    # Main training loop
    for ii in range(int(max_num_training_steps/(num_checkpoint_steps*number_of_evaluations))):
        allx_data, labels, allo_data=transform_data(num_checkpoint_steps*number_of_evaluations,batch_size)
        for jj in range(num_checkpoint_steps*number_of_evaluations):            
            # Compute Adversarial Perturbations
            start = timer()
            end = timer()
            training_time += end - start
            
            nat_dict = {model.x_input: allx_data[(jj%len(allx_data))],
                    model.y_input: labels[(jj%len(allx_data))]}
                # Output to stdout
            if (ii*num_checkpoint_steps*number_of_evaluations+jj) % num_output_steps == 0:
                nat_acc = sess.run(model.accuracy, feed_dict=nat_dict)
                print('Step {}:    ({})'.format((ii*number_of_evaluations*num_checkpoint_steps+jj), datetime.now()))
                print('    training nat accuracy {:.4}%'.format(nat_acc * 100))
                if (ii*number_of_evaluations*num_checkpoint_steps+jj) != 0:
            ...
            # Write a checkpoint
            if (ii*number_of_evaluations*num_checkpoint_steps+jj) % num_checkpoint_steps == 0:
                saver.save(sess,
